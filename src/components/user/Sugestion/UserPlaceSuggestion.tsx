...
export default function UserPlaceSuggestion({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const requestData = {
        ...
        finish_time: questionType === 0 ? `${todayDate}T${finishTime}` : finishTime,
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-2 text-center text-purple-500">나만의 여행 코스 추천</h2>
          ...
          <div className="grid grid-cols-3 gap-2 w-full">
            {['오늘 여행', '일정 여행', '숙박 여행'].map((type, index) => (
              <div
                ...
                className={`h-22 rounded-xl p-3 flex flex-col justify-start gap-5 transition-all duration-200 cursor-pointer border-2 ${questionType === index
                  ? 'border-purple-500 shadow-md shadow-purple-100'
                  : 'border-gray-100 hover:border-gray-200'
                  }`}
              >
                <div className={`text-sm font-bold ${questionType === index ? 'text-purple-500' : 'text-gray-700 dark:text-gray-200'}`}>
                  {type}
                </div>
        ...
        <div className="space-y-4 p-3 bg-gray-50 rounded-xl">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-900 dark:text-white"
            />
          </div>
          ...
          <div className="space-y-2">
              ...
              <div className="bg-white rounded-lg border border-gray-100 p-2">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {['오전', '오후'].map((period, idx) => (
                    <div
                      ...
                      className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${timePeriod === idx
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {timeOptions[timePeriod].map((hour, idx) => (
                      <div
                        ...
                        className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${selectedHour === hour
                          ? 'bg-purple-500 text-white font-medium'
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {minuteOptions.map((minute, idx) => (
                      <div
                        ...
                        className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${selectedMinute === minute
                          ? 'bg-purple-500 text-white font-medium'
                          : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                          }`}
                      >
                ...
                {displayTime && (
                  <div className="text-center mt-2 p-2 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-base font-medium text-purple-500">{displayTime}</div>
                  </div>
                )}
              </div>
            ) : (
                        ...
                        {timeOptions[timePeriod].map((hour, idx) => (
                          <div
                            ...
                            onClick={() => {
                              ...
                              setStartTime(`${dateStr}T${selectedHour}:${minute}`);
                            }}
                            className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${startTime.split('T')[1]?.split(':')[0] === hour
                              ? 'bg-purple-500 text-white font-medium'
                              : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                              }`}
                          >
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {minuteOptions.map((minute, idx) => (
                          <div
                            ...
                            className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${startTime.split('T')[1]?.split(':')[1] === minute
                              ? 'bg-purple-500 text-white font-medium'
                              : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                              }`}
                          >
            ...
            <div className="space-y-2">
              <label htmlFor="finishTime" className="block text-sm font-medium text-gray-700 flex items-center">
                ...
                {questionType === 0 ? '여행 종료 시간' : questionType === 1 ? '여행 종료 시간' : '귀가 날짜/시간'}
              </label>
                ...
                <div className="bg-white rounded-lg border border-gray-100 p-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    {['오전', '오후'].map((period, idx) => (
                      <div
                        key={idx}
                        onClick={() => setEndTimePeriod(idx)}
                        className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${endTimePeriod === idx
                          ? 'bg-purple-500 text-white font-medium'
                          : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                          }`}
                      >
                  ...
                  <div className="mb-3">
                    <div className="text-xs text-gray-500 mb-1 px-1">시간</div>
                    <div className="grid grid-cols-4 gap-2">
                      {timeOptions[endTimePeriod].map((hour, idx) => (
                        <div
                          key={idx}
                          onClick={() => {
                            const minute = finishTime?.split(':')[1] || '00';
                            setFinishTime(`${hour}:${minute}`);
                          }}
                          className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${finishTime?.split(':')[0] === hour
                            ? 'bg-purple-500 text-white font-medium'
                            : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                            }`}
                        >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {minuteOptions.map((minute, idx) => (
                        <div
                          key={idx}
                          onClick={() => {
                            const hour = finishTime?.split(':')[0] || '00';
                            setFinishTime(`${hour}:${minute}`);
                          }}
                          className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${finishTime?.split(':')[1] === minute
                            ? 'bg-purple-500 text-white font-medium'
                            : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                            }`}
                        >
                  ...
                  {finishTime && (
                    <div className="text-center mt-2 p-2 bg-gray-50 rounded-lg">
                      <div className="text-xs text-gray-500 mb-1">선택된 종료 시간</div>
                      <div className="text-base font-medium text-purple-500">{finishTime}</div>
                    </div>
                  )}
                </div>
              ) : (
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          {timeOptions[endTimePeriod].map((hour, idx) => (
                            <div
                              ...
                              className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${finishTime.split('T')[1]?.split(':')[0] === hour
                                ? 'bg-purple-500 text-white font-medium'
                                : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                                }`}
                            >
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {minuteOptions.map((minute, idx) => (
                            <div
                              ...
                              className={`text-center py-2 rounded-lg cursor-pointer text-sm transition-colors ${finishTime.split('T')[1]?.split(':')[1] === minute
                                ? 'bg-purple-500 text-white font-medium'
                                : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                                }`}
                            >
