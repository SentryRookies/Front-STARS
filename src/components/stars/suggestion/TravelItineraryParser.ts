    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (tipMatch) {
            ...
            while (
                ...
                tipContent +=
                    " " +
                    removeDollarSign(improveEmojiHandling(lines[j].trim()));
                j++;
            }
        ...
        if (hotelMatch) {
            try {
                ...
                const addressPart = fullHotelInfo.includes(" - ")
                    ? fullHotelInfo.split(" - ")[0]
                    : fullHotelInfo;

                // 이모지 처리 및 $ 기호 제거
                result.hotelInfo = {
                    description: removeDollarSign(
                        improveEmojiHandling(addressPart)
                    ),
                };
            } catch (err) {
        ...
        if (dayMatch) {
            currentDay = {
                ...
                description: removeDollarSign(
                    improveEmojiHandling(dayMatch[1].trim())
                ), // 이모지 처리 및 $ 기호 제거
                timeItems: [],
            };
        ...
        if (timeMatch) {
            ...
            const contentOnly = removeDollarSign(
                improveEmojiHandling(timeMatch[3].trim())
            ); // 이모지 처리 및 $ 기호 제거
        ...
        if (subItemMatch && currentTimeItem) {
            ...
            const timeInDetailMatch = detail.match(
                /^(🕓|🕙|🕛|🕑|🕕|🕔|🕠|🕞|🕗|🕘|🕚|🕖|⏰)?\s*(\d{1,2}:\d{2}.*?)$/
            );
        ...
        if (currentTimeItem) {
            // 이모지 처리 및 $ 기호 제거
            currentTimeItem.details.push(
                removeDollarSign(improveEmojiHandling(line))
            );
        } else if (currentDay) {
            if (!currentDay.description) {
                currentDay.description = removeDollarSign(
                    improveEmojiHandling(line)
                ); // 이모지 처리 및 $ 기호 제거
            } else {
                currentDay.description +=
                    " " + removeDollarSign(improveEmojiHandling(line)); // 이모지 처리 및 $ 기호 제거
            }
        } else {
            // 이모지 처리 및 $ 기호 제거
            result.additionalInfos.push(
                removeDollarSign(improveEmojiHandling(line))
            );
        }
    }
